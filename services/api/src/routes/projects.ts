...
projectsRouter.get("/:id", async (req, res) => {
  ...
  const [project, videos, jobs, exports, matchData, thumbnails, socialPackages, events, importAudits, clipJobs, transcripts] = await Promise.all([
  ...
  const error = videos.error ?? jobs.error ?? exports.error ?? matchData.error ?? thumbnails.error ?? socialPackages.error ?? events.error ?? importAudits.error ?? clipJobs.error ?? transcripts.error;
  if (error) return res.status(500).json({ error: error.message });
  return res.json({ project: project.data, videos: videos.data ?? [], jobs: jobs.data ?? [], exports: exports.data ?? [], matchData: matchData.data, thumbnails: thumbnails.data ?? [], socialPackages: socialPackages.data ?? [], events: events.data ?? [], importAudits: importAudits.data ?? [], clipJobs: clipJobs.data ?? [], transcripts: transcripts.data ?? [] });
});
