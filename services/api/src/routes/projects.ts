...
projectsRouter.get("/:id", async (req, res) => {
  const supabase = createServiceClient();
  if (!supabase) return res.json({ project: null, videos: [], jobs: [], exports: [], exportJobs: [], packageJobs: [], packageAssets: [], uploadVerifications: [], importJobs: [], matchData: null, thumbnails: [], socialPackages: [] });
  const projectId = req.params.id;
  const [project, videos, jobs, exports, exportJobs, packageJobs, packageAssets, uploadVerifications, importJobs, matchData, thumbnails, socialPackages, events, importAudits, clipJobs, transcripts] = await Promise.all([
    ...
    supabase.from("package_assets").select("*").eq("project_id", projectId).eq("user_id", req.user!.id).order("created_at", { ascending: false }),
    supabase.from("upload_verifications").select("*").eq("project_id", projectId).eq("user_id", req.user!.id).order("created_at", { ascending: false }),
  ...
  const error = videos.error ?? jobs.error ?? exports.error ?? exportJobs.error ?? packageJobs.error ?? packageAssets.error ?? uploadVerifications.error ?? importJobs.error ?? matchData.error ?? thumbnails.error ?? socialPackages.error ?? events.error ?? importAudits.error ?? clipJobs.error ?? transcripts.error;
  if (error) return res.status(500).json({ error: error.message });
  return res.json({ project: project.data, videos: videos.data ?? [], jobs: jobs.data ?? [], exports: exports.data ?? [], exportJobs: exportJobs.data ?? [], packageJobs: packageJobs.data ?? [], packageAssets: packageAssets.data ?? [], uploadVerifications: uploadVerifications.data ?? [], importJobs: importJobs.data ?? [], matchData: matchData.data, thumbnails: thumbnails.data ?? [], socialPackages: socialPackages.data ?? [], events: events.data ?? [], importAudits: importAudits.data ?? [], clipJobs: clipJobs.data ?? [], transcripts: transcripts.data ?? [] });
});
