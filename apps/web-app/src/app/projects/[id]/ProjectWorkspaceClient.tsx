...
export function ProjectWorkspaceClient({ projectId, tab }: { projectId: string; tab: WorkspaceTab }) {
  ...
  if (tab === "exports") return <section className="grid"><FormatSelector /><OutputFormatCards /><ReadinessChecklist />{data.exports.length ? <JsonCard title="Exports" value={data.exports} /> : <div className="card"><EmptyState label="export" /></div>}{data.exportJobs?.length ? <JsonCard title="MP4 Conversion Jobs" value={data.exportJobs} /> : <div className="card"><EmptyState label="conversion job" /></div>}</section>;
