...
type WorkspaceTab = "overview" | "imports" | "timeline" | "match-data" | "highlights" | "captions" | "commentary" | "thumbnail" | "social-pack" | "exports" | "debug";
  ...
  useEffect(() => {
    ...
    const loadProject = (showLoading: boolean) => {
      ...
      .then((response) => {
        ...
        const active = [...response.jobs, ...(response.exportJobs ?? []), ...(response.importJobs ?? [])].some((job) => ["queued", "processing"].includes(String(job.status)));
